...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(uri !=null){
            ...
            if(localBundle!=null){
                String t_path = localBundle.getString("path");//不懂
                if(t_path!=null&&!"".equals(t_path)){//t_path不等于空和什么鬼 和另一种形式的不等于空？
                    //用空对象来比较，这个！“”。equal（）下比对可以等于是null
                    //而直接用equals（“”）；来比对的话，空对象是会报错的。
                    mPath = t_path;
                }
            }
        }
        ...
        holder.addCallback(new SurfaceHolder.Callback() {
            ...
            public void surfaceCreated(SurfaceHolder holder) {
                ...
                //在指定了MediaPlayer播放的容器后，我们就可以用prepare或者prepaAsync来准备播放了
                player.prepareAsync();
            }
            @Override
        ...
        //为了可以播放视频或者Camera预览，我们需要指定他的buffer类型
        ...
        player.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
            ...
            public void onPrepared(MediaPlayer mp) {
                //当prepare完成后，该方法触发，在这里我们直接播放视频
                //首先取得video的宽和高
                vWidth = player.getVideoWidth();
                vHeight = player.getVideoHeight();
                if(vWidth>currDisplay.getWidth()||vHeight >currDisplay.getHeight()){
                    //如果vi的宽或者高超出了当前屏幕大小，则要进行缩放
        ...
        try{
            //然后指定需要播放文件的引导，初始化Mediaplayer
            if(!mPath.equals("")){
                title.setText(mPath.substring(mPath.lastIndexOf("/")+1));
                player.setDataSource(mPath);
            }else{
                AssetFileDescriptor afd = this.getResources().openRawResourceFd(R.raw.papi1);//啥
    ...
    private final Handler handler = new Handler(){
        public void handlerMessage (Message msg){
            switch (msg.what){
                case 1 :
                    //Time类需要导入java。sql包
                    Time progess = new Time(player.getCurrentPosition());
                    Time allTime = new Time(player.getDuration());
                    String timeStr = progess.toString();
